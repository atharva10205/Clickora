        ...
        <div className="bg-[#111111] border border-gray-800/70 p-6 rounded-xl mb-5">
            <div className="flex items-center justify-between mb-1">
                ...
                <div className="text-right">
                    <span className="text-xl font-bold font-mono" style={{ color: accent }}>{total.toLocaleString()}</span>
                    <p className="text-xs text-gray-600">total clicks</p>
                </div>
            </div>
            <div className="relative w-full" style={{ paddingBottom: '28%' }}>
                <svg viewBox={`0 0 ${W} ${H}`} className="absolute inset-0 w-full h-full overflow-visible" preserveAspectRatio="none">
                    <defs>
                    ...
                    {Array.from({ length: 5 }).map((_, i) => (
                        <line key={i} x1={padL} y1={padT + (i / 4) * chartH} x2={padL + chartW} y2={padT + (i / 4) * chartH} stroke="#1f1f1f" strokeWidth="1" />
                    ))}
                    ...
                    {hoveredIdx !== null && (
                        <line x1={pts[hoveredIdx].x} y1={padT} x2={pts[hoveredIdx].x} y2={padT + chartH} stroke="#444" strokeWidth="1" strokeDasharray="3 3" />
                    )}
                    {pts.map((pt, i) => (
                        <g key={i}>
                            <rect x={pt.x - 30} y={padT} width={60} height={chartH} fill="transparent" onMouseEnter={() => setHoveredIdx(i)} onMouseLeave={() => setHoveredIdx(null)} style={{ cursor: 'crosshair' }} />
                            <circle cx={pt.x} cy={pt.y} r={hoveredIdx === i ? 4 : 2.5} fill={hoveredIdx === i ? accent : a(0.4)} stroke={hoveredIdx === i ? a(0.3) : 'none'} strokeWidth="6" style={{ transition: 'r 0.15s' }} pointerEvents="none" />
                            {hoveredIdx === i && (() => {
                                ...
                                return (
                                    <g pointerEvents="none">
                                        ...
                                        <text x={tx + tw / 2} y={ty + 13} textAnchor="middle" fill="#aaa" fontSize="9" fontFamily="monospace">{clicksChart[i].date}</text>
                                        <circle cx={tx + 14} cy={ty + 27} r="3" fill={accent} />
                                        <text x={tx + 22} y={ty + 31} fill={accent} fontSize="11" fontWeight="700" fontFamily="monospace">{clicksChart[i].count} {clicksChart[i].count === 1 ? 'click' : 'clicks'}</text>
                                    </g>
                                );
                            })()}
                            <text x={pt.x} y={padT + chartH + 18} textAnchor="middle" fill={hoveredIdx === i ? '#888' : '#3a3a3a'} fontSize="9" fontFamily="monospace">{clicksChart[i].date}</text>
                        </g>
                    ))}
                </svg>
            </div>
...
const DeleteModal = ({
    ...
    const g = parseInt(accent.slice(3, 5), 16);
    const b = parseInt(accent.slice(5, 7), 16);
    const hAlpha = (op: number) => `rgba(${r},${g},${b},${op})`;
    ...
    return (
        ...
        >
            <div
                className="flex flex-col w-[360px] rounded-2xl px-8 py-8"
            ...
            >
                ...
                <p className="font-mono text-xs mb-6" style={{ color: 'rgba(255,255,255,0.35)' }}>
                    {hasBalance
                        ? 'You have unclaimed earnings. Withdraw them first, then this site will be deleted.'
                        : 'This action is permanent and cannot be undone.'}
                </p>

                {/* Balance card (only shown when has balance) */}
                {hasBalance && (
                    <div
                    ...
                    >
                        <span className="font-mono text-xs" style={{ color: 'rgba(255,255,255,0.35)', textTransform: 'uppercase', letterSpacing: '0.1em' }}>
                            Claimable
                        </span>
                ...
                <div className="flex gap-3">
                    <button
                        ...
                        className="flex-1 py-2.5 rounded-xl text-sm font-mono font-medium transition-all duration-150 disabled:opacity-40"
                        style={{
                            background: '#161616',
                    ...
                    <button
                        ...
                        disabled={isProcessing}
                        className="flex-1 py-2.5 rounded-xl text-sm font-mono font-semibold transition-all duration-150 disabled:opacity-50 disabled:cursor-not-allowed"
                        style={{
...
export default function WebsiteDetailPage() {
    ...
    return (
        <>
            ...
            {toast && (
                <div
                    className="fixed bottom-6 left-1/2 -translate-x-1/2 z-50 flex items-center gap-3 px-5 py-3 rounded-lg shadow-xl"
                    style={{
                        background: '#1a1a1a',
                        border: toastType === 'success' ? `1px solid ${alpha(0.35)}` : '1px solid rgba(239,68,68,0.3)',
                    }}
                >
                    <div className="w-1.5 h-1.5 rounded-full shrink-0" style={{ background: toastType === 'success' ? accent : '#ef4444' }} />
                    <p className="text-sm font-mono" style={{ color: toastType === 'success' ? accent : '#fca5a5' }}>{toast}</p>
                    ...
                    <div className="max-w-4xl mx-auto px-6">
                        <div className="flex items-center justify-between h-14">
                            <div className="flex items-center gap-3">
                                <button onClick={() => router.back()} className="w-7 cursor-pointer h-7 rounded-md bg-[#1a1a1a] border border-[#2a2a2a] flex items-center justify-center hover:border-gray-600 hover:text-gray-300 text-gray-500 transition-all duration-150">
                                    <ChevronLeft className="w-3.5 h-3.5" />
                                </button>
                                <span className="text-gray-700">|</span>
                                <span className="text-white font-semibold text-sm tracking-tight">Publisher</span>
                                <span className="text-gray-700">|</span>
                                <span className="text-gray-500 text-sm">{editing ? 'Edit Website' : websiteName || 'Website'}</span>
                            </div>
                            <div className="flex items-center gap-3">
                                {userName && <span className="text-xs text-gray-500 font-mono hidden sm:block">{userName}</span>}
                                <div className="w-7 h-7 rounded-md bg-[#1a1a1a] border border-[#2a2a2a] overflow-hidden flex items-center justify-center">
                                    {userImage ? <img src={userImage} alt={userName} className="w-full h-full object-cover" /> : <User className="w-4 h-4 text-gray-500" />}
                                </div>
                                {editing ? (
                                    <>
                                        <button onClick={() => setEditing(false)} className="flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-xs text-gray-500 hover:text-gray-200 border border-white/[0.06] hover:border-white/[0.12] transition-all duration-150">
                                            <X className="w-3 h-3" /> Cancel
                                        </button>
                                        <button
                                            onClick={handleSave}
                                            disabled={saving}
                                            className="flex items-center gap-1.5 px-4 py-1.5 rounded-lg text-xs font-semibold transition-all duration-150 disabled:opacity-40"
                                    ...
                                    <>
                                        <button onClick={() => setEditing(true)} className="flex cursor-pointer items-center gap-1.5 px-3 py-1.5 rounded-lg text-xs text-gray-500 hover:text-gray-200 border border-white/[0.06] hover:border-white/[0.12] transition-all duration-150">
                                            Edit
                                        </button>
                                        <button onClick={() => {alert("Under Devlopement") }} className="flex cursor-pointer items-center gap-1.5 px-3 py-1.5 rounded-lg text-xs transition-all duration-150 text-red-500/60 hover:text-red-400 border border-red-500/10 hover:border-red-500/25 hover:bg-red-500/[0.04]">
                                            Delete
                                        </button>
                                    </>
                                )}
                        ...
                        <>
                            ...
                            <div className="flex items-center justify-between">
                                <div>
                                    ...
                                    <h1 className="text-2xl font-bold text-white tracking-tight">{websiteName || websiteURL}</h1>
                                    <p className="text-xs text-gray-700 font-mono mt-0.5">{websiteURL}</p>
                                </div>
                                <span
                                    className="text-xs px-2.5 py-1 rounded font-mono tracking-wide border"
                                    style={{
                                ...
                                >
                                    {siteStatus === 'ACTIVE' && (
                                        <span className="inline-block w-1.5 h-1.5 rounded-full mr-1.5 align-middle animate-pulse" style={{ background: accent }} />
                                    )}
                                    {siteStatus}
                                </span>
                            </div>
                            ...
                            {analytics && (
                                <div className="grid grid-cols-2 lg:grid-cols-3 gap-3">
                                    {statCards.map(card => (
                                        <div key={card.label} className="bg-[#0e0e0e] rounded-xl p-5 border border-white/[0.05] hover:border-white/[0.1] transition-all duration-300">
                                            <div className="flex items-start justify-between mb-4">
                                                <p className="text-[10px] text-gray-600 uppercase tracking-[0.15em]">{card.label}</p>
                                                <card.icon className="w-3.5 h-3.5 text-gray-700" />
                                            </div>
                                            <p className="text-2xl font-bold tabular-nums font-mono" style={{ color: card.accent ? accent : undefined }}>
                                                {card.value}
                                            </p>
                                        </div>
                                    ))}
                            ...
                            <div className="bg-[#111111] border border-gray-800/70 rounded-xl overflow-hidden">
                                <div className="px-6 py-4 border-b border-gray-800/60">
                                    <h2 className="text-[10px] text-gray-600 uppercase tracking-[0.15em] font-mono">Site Details</h2>
                                </div>
                                <div className="p-6 grid grid-cols-1 sm:grid-cols-2 gap-6">
                                        ...
                                        <div className="flex flex-wrap gap-1.5 mt-2">
                                            {selectedNiches.length > 0
                                                ? selectedNiches.map((n, i) => (
                                                    <span key={i} className="bg-[#161616] border border-white/[0.06] text-gray-400 px-2.5 py-1 rounded-md text-[11px]">{n}</span>
                                                ))
                                    ...
                                    <div>
                                        ...
                                        <div className="flex flex-wrap gap-1.5 mt-2">
                                            {keywords.length > 0
                                                ? keywords.map((k, i) => (
                                                    <span key={i} className="bg-[#161616] border border-white/[0.06] text-gray-400 px-2.5 py-1 rounded-md text-[11px] font-mono">{k}</span>
                                                ))
                                                : <span className="text-gray-700 text-sm">—</span>
                                            }
                                        </div>
                                    ...
                                    <div>
                                        ...
                                        <p className="text-xs text-gray-500 font-mono break-all">{walletAddress ? `${walletAddress.slice(0, 8)}…${walletAddress.slice(-8)}` : '—'}</p>
                                    </div>
                                    <div>
                                        <p className="text-[10px] text-gray-600 uppercase tracking-widest mb-1 font-mono">URL</p>
                                        <p className="text-xs text-gray-500 font-mono">{websiteURL}</p>
                                    </div>
                                </div>
                            </div>
                        </>
                    ...
                    {editing && (
                        <div className="bg-[#111111] border border-gray-800/70 rounded-xl overflow-hidden">
                            <div className="px-8 py-6 border-b border-gray-800/60">
                                ...
                                <h1 className="text-xl font-semibold text-white tracking-tight mb-1">Edit website details</h1>
                            ...
                            <div className="p-8 space-y-8">
                                ...
                                <div>
                                    ...
                                    <div className="relative">
                                        ...
                                        <input type="text" value={websiteURL} disabled className={`${inputClass(false)} pl-16 opacity-50 cursor-not-allowed`} />
                                    </div>
                                </div>
                                ...
                                <div>
                                    ...
                                    <div className="relative">
                                        ...
                                        <input type="text" value={walletAddress} disabled className={`${inputClass(false)} pl-10 opacity-50 cursor-not-allowed`} />
                                    </div>
                                    <p className="mt-1.5 text-xs text-gray-600 font-mono">
                                        Change it in{' '}
                                        <span onClick={() => router.push('/Publisher/Settings')} className="text-gray-300 cursor-pointer hover:underline">Settings →</span>
                                    </p>
                                </div>
                                ...
                                <div>
                                    <div className="flex items-center justify-between mb-2">
                                        <label className={`text-xs uppercase tracking-widest font-mono ${errors.selectedNiches ? 'text-red-400' : 'text-gray-600'}`}>Website Niches</label>
                                        <div className="flex items-center gap-3">
                                            <span className="text-xs text-gray-600 font-mono">{selectedNiches.length} / 15</span>
                                            {selectedNiches.length > 0 && (
                                                <button onClick={() => setSelectedNiches([])} className="text-xs cursor-pointer text-gray-500 hover:text-gray-300 transition-colors font-mono">Clear all</button>
                                            )}
                                        </div>
                                    </div>
                                    <p className={`text-xs mb-4 ${errors.selectedNiches ? 'text-red-400' : 'text-gray-600'}`}>Choose up to 15 niches</p>
                                        ...
                                        <div className="flex flex-wrap gap-2 mb-4">
                                            {selectedNiches.map((niche, i) => (
                                                <div key={i} className="flex items-center gap-1.5 bg-[#0d0d0d] border border-gray-800/50 rounded-lg px-3 py-1.5 text-xs text-gray-300">
                                                    <span>{niche}</span>
                                                    <button onClick={() => removeNiche(niche)} className="text-gray-600 cursor-pointer hover:text-gray-300 transition-colors"><X className="w-3 h-3" /></button>
                                                </div>
                                            ))}
                                        </div>
                                    )}
                                        ...
                                        {niches.map(niche => {
                                            ...
                                            return (
                                                ...
                                                >
                                                    ...
                                                    <span className={`text-[10px] font-medium text-center leading-tight ${isSelected ? 'text-white' : 'text-gray-500'}`}>{niche.label}</span>
                                                </button>
                                            );
                                        })}
                                    </div>
                                    ...
                                    >
                                        {keywords.map((kw, i) => (
                                            <span key={i} className="flex items-center gap-1 bg-[#1c1c1c] border border-gray-700/60 text-gray-300 px-2 py-0.5 rounded text-xs font-mono">
                                                {kw}
                                                <button onClick={() => removeKeyword(i)} className="text-gray-600 hover:text-gray-300 transition-colors"><X className="w-2.5 h-2.5" /></button>
                                            </span>
                                        ))}
                                        <input
                                            type="text"
                            ...
                            <div className="px-8 py-5 bg-[#0d0d0d] border-t border-gray-800/60 flex items-center justify-between">
                                <button onClick={() => setEditing(false)} className="px-5 py-2.5 rounded-lg bg-[#161616] border border-gray-800/60 text-gray-400 text-sm font-medium hover:text-gray-200 hover:border-gray-600 transition-all duration-150">
                                    Cancel
                                </button>
                                <button
                                    onClick={handleSave}
                                    disabled={saving}
                                    className="px-6 py-2.5 rounded-lg bg-[#161616] text-gray-200 text-sm font-semibold hover:-translate-y-0.5 active:translate-y-0 transition-all duration-200 disabled:opacity-40 disabled:cursor-not-allowed"
                                    style={{ border: `1px solid ${alpha(0.25)}` }}
                                    onMouseEnter={e => { if (!saving) { e.currentTarget.style.borderColor = accent; e.currentTarget.style.boxShadow = `0 0 18px ${alpha(0.12)}`; e.currentTarget.style.color = '#ffffff'; } }}
...
//     }[]>([]);
//     const [publisherWalletAddress, setPublisherWalletAddress] = useState<string | null>(null);
...
//     const claimableSOL = earningsRecords.reduce((sum, r) => sum + r.claimable_amount, 0) / 1_000_000_000;
...
//         const fetchData = async () => {
//             try {
//                 const [siteRes, earningsRes] = await Promise.all([
//                     fetch(`/api/crud/Publisher/Publisher_website/${id}`),
//                     fetch('/api/crud/Publisher/Earning'),
//                 ]);
...
//                 setEarningsRecords(
//                     (earningsData.earningsRecords ?? []).filter(
//                         (r: any) => r.publisher === data.site?.wallet_address
//                     )
//                 );
//                 setPublisherWalletAddress(earningsData.publisher?.wallet_address ?? null);
//             } catch (err) {
//                 console.error('Fetch failed:', err);
//             } finally {
//                 setLoading(false);
...
// const claimEarnings = async (): Promise<boolean> => {
//         if (!wallet.connected) {
...
//             const publisherPubkey = new PublicKey(publisherWalletAddress!);

//             let claimSources = postResults.length > 0
//                 ? postResults
//                 : (() => {
...
//                         if (!adMap.has(e.ad)) adMap.set(e.ad, { ad: e.ad, advertiser: e.advertiser, claimable_amount: 0 });
//                         adMap.get(e.ad)!.claimable_amount += e.claimable_amount;
//                     }
//                     return [...adMap.values()];
//                 })();

//             if (claimSources.length === 0) return true;
...
//                     [Buffer.from('ad'), advertiserPubkey.toBuffer(), adIdBytes], program.programId
//                 );
//                 const [vaultPda] = PublicKey.findProgramAddressSync(
//                     [Buffer.from('vault'), advertiserPubkey.toBuffer(), adIdBytes], program.programId
//                 );
//                 const ix = await program.methods.claim(new BN(e.claimable_amount)).accounts({
//                     vault: vaultPda, ad: adPda, advertiser: advertiserPubkey,
//                     publisher: publisherPubkey, systemProgram: SystemProgram.programId,
//                 }).instruction();
//                 transaction.add(ix);
//             }

//             if (transaction.instructions.length === 0) return true;
...
//             if (simResult.err) {
//                 showToast(`Simulation failed: ${JSON.stringify(simResult.err)}`, 'error');
//                 return false;
//             }
...
//                 showToast(`Transaction failed: ${JSON.stringify(confirmation.value.err)}`, 'error');
//                 return false;
//             }
...
//                 method: 'PATCH',
//                 headers: { 'Content-Type': 'application/json' },
//                 body: JSON.stringify({ adIds: claimSources.map(r => r.ad) }),
//             });
//             return true;
//         } catch (error: any) {
...
//     const handleClaimAndDelete = async () => {
//         setDeletePhase('claiming');
//         const claimed = await claimEarnings();
//         if (!claimed) { setDeletePhase('idle'); return; }
//         setDeletePhase('deleting');
//         const ok = await deleteSite();
//       if (ok) {
//             router.push('/Publisher/Dashboard');
//         } else {
//             showToast('Earnings claimed but failed to delete. Try again.', 'error');
//             setDeletePhase('idle');
//             setShowDeleteModal(false);
//         }
//     };
...
//                     onClaimAndDelete={handleClaimAndDelete}
...
//                                             className="flex cursor-pointer items-center gap-1.5 px-3 py-1.5 rounded-lg text-xs transition-all duration-150 text-red-500/60 hover:text-red-400 border border-red-500/10 hover:border-red-500/25 hover:bg-red-500/[0.04]"
//                                         >
//                                             Delete
//                                         </button>
//                                     </>
